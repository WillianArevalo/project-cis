        ...
        <ul class="inline-flex items-stretch -space-x-px">
            @if ($paginator->onFirstPage())
                <li aria-disabled="true" aria-label="@lang('pagination.previous')">
                    <span
                        class="ml-0 flex h-full cursor-not-allowed items-center justify-center rounded-l-lg border border-zinc-400 bg-white px-3 py-1.5 text-zinc-500 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-400">
                    ...
                    @foreach ($element as $page => $url)
                        @if ($page == $paginator->currentPage())
                            <li aria-current="page"><span
                                    class="border-primary-300 z-10 flex items-center justify-center border bg-blue-600 px-3 py-2 text-sm leading-tight text-white dark:border-zinc-800 dark:bg-blue-700 dark:text-white">{{ $page }}</span>
                            </li>
                        @else
                            <li><a href="{{ $url }}"
            ...
            @else
                <li aria-disabled="true" aria-label="@lang('pagination.next')">
                    <span
                        class="flex h-full cursor-not-allowed items-center justify-center rounded-r-lg border border-zinc-400 bg-white px-3 py-1.5 leading-tight text-zinc-500 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-400">
                        <span class="sr-only">Next</span>
                        <svg class="h-5 w-5" aria-hidden="true" fill="currentColor" viewbox="0 0 20 20"
                            xmlns="http://www.w3.org/2000/svg">
                            <path fill-rule="evenodd"
