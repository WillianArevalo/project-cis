        ...
        <form action="{{ Route('admin.proyectos.update', $proyecto->id) }}" class="w-full" method="POST"
            ...
            <div class="mt-4">
                ...
                <div id="list-specific-objetives-inputs">
                    @if ($proyecto->specificObjetives->count() > 0)
                        @foreach ($proyecto->specificObjetives as $objective)
                            <input type="hidden" name="specific_objectives[]"
                                value="{{ $objective->specific_objective }}" />
                        @endforeach
                    @endif
                </div>
            </div>
            ...
            <div class="mt-4">
                <x-input type="text" name="location" placeholder="Ingresa la ubicación del proyecto"
